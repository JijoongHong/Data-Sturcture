...
polynominal poly_mul1(polynominal A, polynominal B){
    ...
    for (int i = 0; i < A.degree+1; i++){
        ...
        for (int j = 0; j < B.degree+1; j++){
            ...
            // 이미 해당 차수에 값이 있어 겹치는 경우, 계수가 0인 경우 카운트 증가
            // 계수가 0인 경우 -> 예) 수식1의 계수가 0인 경우에 0값이 추가됨
            // 추후 동일 지수값이 새로이 추가될 시 지수에 이미 값이 존재한다고 판단하는 경우 고려
...
int main(){
    ...
    int a_list[6], b_list[6], a_len=0, b_len=0; // 항의 값 저장용 배열, 항의 개수 저장용 배열 선언
    
    printf("수식 1을 입력하세요 ");
    scanf("%[^\n]", a_input);
    
    char *temp = strtok(a_input, " ");          // 띄어쓰기를 기준으로 분할하여 저장
    while (temp != NULL){
        if(a_len > 6){                          // 5차항을 초과하면 에러 발생
            fprintf(stderr, "항의 개수가 너무 많음\n");
            exit(1);}
        
        a_list[a_len] = atoi(temp);
        temp = strtok(NULL, " ");
        a_len++;
    }
    ...
    char *temp2 = strtok(b_input, " ");         // 띄어쓰기를 기준으로 분할하여 저장
    while (temp2 != NULL){
        if(b_len > 6){                          // 5차항을 초과하면 에러 발생
            fprintf(stderr, "항의 개수가 너무 많음\n");
            exit(1);}
        
        b_list[b_len] = atoi(temp2);
        temp2 = strtok(NULL, " ");
        b_len++;
    }
    
    
    polynominal a;  // 구조체 선언 후에 수식1의 값 입력
    a.degree = a_len -1 ;
    for (int i = 0; i<a_len; i++){
        a.coef[i] = a_list[i];
    }
    
    polynominal b;  // 구조체 선언 후에 수식2의 값 입력
    b.degree = b_len - 1;
